import { Button, Checkbox, Field, Input, RadioGroup, Stack, Text, VStack } from "@chakra-ui/react";
  ...
  const inputs = (
    <>
      ...
      <Stack w="full" gap={4} align="flex-start">
        ...
        <RadioGroup.Root value={roleValue} onValueChange={(e) => setRoleValue(e.value as Role)}>
          <VStack gap={6} alignItems="flex-start">
            {allRoles.map((item) => (
              <RadioGroup.Item key={item.key} value={item.key}>
                <RadioGroup.ItemHiddenInput />
                <RadioGroup.ItemIndicator />
                <RadioGroup.ItemText fontWeight="normal" textTransform="capitalize">
                  {item.label}
                </RadioGroup.ItemText>
              </RadioGroup.Item>
            ))}
