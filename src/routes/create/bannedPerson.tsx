...
function RouteComponent() {
  ...
  const createBannedPersonHandler = async () => {
    ...
    createBannedPersonDto.append("endDate", dayjs(`${year}/${month}/${day}`).toISOString());
  ...
  const inputs = (
    <>
      <FileUpload.Root onFileChange={(e) => setImage(e.acceptedFiles[0])} gap="1">
  ...
  const button = (
    <Button
      disabled={
        image === undefined || name === "" || reason === "" || endDate.length === 0 || noneChecked
      }
