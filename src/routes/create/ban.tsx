...
export const Route = createFileRoute("/create/ban")({
  ...
  loader: async () => {
    const [allBannedPeople, allVenues] = await Promise.all([getAllBannedPeople(), getAllVenues()]);

    return {
      allBannedPeople: isErrorCheck(allBannedPeople) ? [] : (allBannedPeople as BannedPerson[]),
      allVenues: isErrorCheck(allBannedPeople) ? [] : (allVenues as Venue[]),
    };
  },
});
...
function RouteComponent() {
    ...
    <>
      ...
      <Stack w="full" gap={4} align="flex-start">
        ...
        <Checkbox.Root
          ...
          onCheckedChange={(e) =>
            setVenueValues((current) => current.map((v) => ({ ...v, checked: !!e.checked })))
          }
        >
          ...
          <Checkbox.Root
            ...
            onCheckedChange={(e) =>
              setVenueValues((current) =>
                current.map((v, i) => (i === index ? { ...v, checked: !!e.checked } : v)),
              )
            }
          >
            <Checkbox.HiddenInput />
  ...
  const button = (
    <Button
      disabled={!selectedBannedPerson || reason === "" || endDate.length === 0 || noneChecked}
