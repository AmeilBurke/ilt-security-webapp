import { Button, CloseButton, Field, FileUpload, Input, InputGroup, Text } from "@chakra-ui/react";
...
function RouteComponent() {
  ...
  const inputs = (
    <>
      <FileUpload.Root onFileChange={(e) => setVenueImage(e.acceptedFiles[0])} gap="1">
          ...
          <Input asChild>
            <FileUpload.Trigger>
              <FileUpload.Context>
                {({ acceptedFiles }) => (
                  <Text lineClamp={1}>
                    {acceptedFiles.length > 0 ? acceptedFiles[0].name : "Upload image of venue"}
                  </Text>
                )}
              </FileUpload.Context>
            </FileUpload.Trigger>
  ...
  const button = (
    <Button
      disabled={venueImage === undefined || name === "" || address === "" || phone === ""}
