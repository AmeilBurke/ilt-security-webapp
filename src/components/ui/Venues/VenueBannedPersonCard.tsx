...
const VenueBannedPersonCard = ({
	person,
	venueId,
}: VenueBannedPersonCardProps) => {
	const hasActiveBanFromVenue = (): boolean => {
		return (
			person.bans?.some((ban) => {
				const banVenueIds = ban.venueBans.filter(
					(vb) => vb.venueId === venueId,
				);
	...
	return (
		<VStack h="100%" align="flex-start" gap={2}>
			...
			>
				...
				{hasActiveBanFromVenue() ? <Badge colorPalette="red">Has Active ban</Badge> : <Badge>Ban Expired</Badge>}
			</VStack>
		</VStack>
	);
};
