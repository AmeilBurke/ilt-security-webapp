...
import type { Ban, BannedPerson, DialogMode, Venue, VenueSelection } from "@/utils/interfaces";
...
const computeVenueSelections = (pendingBan: Ban, venues: Venue[]): VenueSelection[] => {
  const bannedVenueIds = new Set(pendingBan.venueBans.map((venueBan) => venueBan.venueId));
...
const TabPendingBans = ({ pendingBans, venues, allBanned }: TabPendingBansProps) => {
  ...
  const openDialog = (pendingBan: Ban) => {
    ...
    setSelectedBannedPerson(allBanned.find((person) => person.id === pendingBan.personId));
  ...
  const confirmDeleteDialogFooter = (
    <>
      ...
      <Button
        onClick={() =>
          selectedPendingBan?.id ? handleDeletePendingBan(selectedPendingBan.id) : null
        }
        colorPalette="red"
      >
        Delete
  ...
  const handleApprovePendingBan = async (pendingBanId: string) => {
    ...
    const updateBanDto: UpdateBanDto = {
      ...
      endDate:
        endDate.length > 0
          ? dayjs(`${endDate[0].year}/${endDate[0].month}/${endDate[0].day}`).toISOString()
          : undefined,
  ...
  const confirmApproveDialogTrigger = <Button colorPalette="current">Approve</Button>;
  const confirmApproveDialogBody = <Text>Press approve again to confirm the ban.</Text>;
  const confirmApproveDialogFooter = (
    <>
      ...
      <Button
        onClick={() => selectedPendingBan && handleApprovePendingBan(selectedPendingBan.id)}
        ...
        <Portal>
          ...
          <Dialog.Positioner>
            <Dialog.Content>
              ...
              <Dialog.Header>
                <Dialog.Title>Ban Details For {selectedBannedPerson?.name}</Dialog.Title>
              </Dialog.Header>
              <Dialog.Body>
                <VStack gap={8}>
